	...
	public static String[] getTokens(String str, char delim)
	{
		...
		for (; i<str.length(); i++)
		{
			...
			if (tmp == '\'')
			{
				...
				if (isNoInfo(strTmp) == false) {System.err.println("only blank may exist between a comma and the first one of a pair of single quote. Check bus name section.."); System.exit(1);}
				else strTmp = "";
				strTmp += tmp;
				...
				if (ndxComma != -1) {
					if (isNoInfo(str.substring(0, ndxComma)) == false) {System.err.println("only blank may exist between the second one of a pair of single quote and a comma. Check bus name section.."); System.exit(1);}
